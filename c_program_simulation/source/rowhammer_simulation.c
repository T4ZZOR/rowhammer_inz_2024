...
int main(){
    ...
    int counter = 0;
    int randomError = rand() % data_elem_size; // generate random index to simulate bit-flip
    printf("RANDOMMMMMMM %d", randomError);
    ...
    while(flag==0){
        ...
        if(attack_mode == 0){
            ...
            compareData(data, size, copy_data);
        }
        ...
        if(dataAttackIndex_1 >= randomError){
            generateSimulationBitFlip(data, dataAttackIndex_1);
        }
    }
    free(data);
    free(copy_data);
